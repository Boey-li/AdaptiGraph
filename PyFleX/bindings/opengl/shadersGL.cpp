...
void InitRender(const RenderInitOptions& options)
{
    ...
    char font_path[100];
    make_path(font_path, "/data/DroidSans.ttf");
	imguiRenderGLInit(GetFilePathByPlatform(font_path).c_str());
...
void DrawPlane(const Vec4& p, bool color, float scale_size);
...
{ 
	...
	if (program == GLint(s_diffuseProgram))
	{
        ...
        make_path(texture_path, "/data/cloth-texture-5.jpg");
		static RenderTexture* tex = CreateRenderTexture(texture_path);

		RenderMaterial mat;
		mat.frontColor = Vec3(1.0f);
		mat.backColor = Vec3(1.0f);
		mat.colorTex = tex;
	...
	if (program == GLint(s_diffuseProgram))
	{
		...
		SetMaterial(Matrix44::kIdentity, RenderMaterial());
	}
#endif
}
...
void DemoContextOGL::drawPlane(const Vec4& p, bool color, float scale_size)
{
	OGL_Renderer::DrawPlane(p, color, scale_size);
}

void DemoContextOGL::drawPlanes(Vec4* planes, int n, float bias, float scale_size)
{
	OGL_Renderer::DrawPlanes(planes, n, bias, scale_size);
}
