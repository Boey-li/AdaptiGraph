...
def sample_gripper_config():
    ...
    x = np.cos(angle) * dis
    z = np.sin(angle) * dis
    d = np.random.rand() * 0.3 + 0.4    # (0.6, 0.8)
    return x, z, d
...
scale = 13.
globalStiffness = 1.0
clusterPlasticThreshold = 0.0015
clusterPlasticCreep = 0.15
draw_mesh = 1

scene_params = np.array([
    scale, globalStiffness, clusterPlasticThreshold, clusterPlasticCreep,
    draw_mesh])

pyflex.set_scene(14, scene_params, 0)
