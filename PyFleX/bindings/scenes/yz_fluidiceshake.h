...
public:
	...
	void Initialize(py::array_t<float> scene_params, int thread_idx = 0)
	{
		...
		float px_r = ptr[7];    // position x
		float py_r = ptr[8];	// position y
		float pz_r = ptr[9];	// position z
		float sx_r = ptr[10];    // size x
		float sy_r = ptr[11];   // size y
		float sz_r = ptr[12];   // size z
        float invMass = ptr[13];
		float draw_mesh = ptr[16];
        ...
        CreateParticleShape(
                ...
                NvFlexMakePhase(group++, 0), true, 0.0f, 0.0f, 0.0f, Vec4(1.0f, 1.0f, 0.0f, 0.0f));
